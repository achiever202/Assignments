...
/* 
 * Changes made here for lottery scheduling and multi level feedback queue scheduling.
 * The number of scheduling queues (NR_SCHED_QUEUES) is increased to 19.
 * In lottery scheduling, the user processes are kept in queue 17, and a lottery is done.
 * The process that wins the lottery is moved to queue 16, thus, increasing its priority.
 * In multi level feedback queue scheduling, the user level processes are kept in queues 16, 17 and 18.
 * The maximum priortity for a user process (MAX_USER_Q) is also changed to 16 for both the scheduling processes. 
 */
...
/* Changes for lottery scheduling and multilevel queue scheduling end here. */
