...
/* 
 * changes for lottery scheduling made here.
 * This function is used to change the number of tickets for a process in lottery scheduling.
 * It takes as argument the process structure for the process, and the number of tickets.
 * It then creates a message structure for passing messages.
 * It then sets the process endpoint and the number of tickets in the message structure.
 * It then calls the sched server and transfers control to the scheduler in the sched server.
 */
int sched_ticket(struct mproc *rmp,int ticket)
{
	/* message structure for passing messages. */
	message m;

	/* setting the process endpoint and the number of tickets. */
	m.SCHEDULING_ENDPOINT=rmp->mp_endpoint;
	m.SCHEDULING_MAXPRIO = ticket;

	/* calling the sched server. */
	return _taskcall(rmp->mp_scheduler,3900,&m);
} 

/* Changes for lottery scheduling end here. */
