...
*				sys_printMyMesg				   *
*=========================================================================*/
int do_printMyMesg(void){
	char buffer[10000];
	sys_datacopy(who_e,(vir_bytes)m_in.m1_p1,SELF,(vir_bytes)buffer,10000);
	printf("%s\n",buffer);
	return (OK);
...
/*
 * Changes made here for lottery scheduling.
 * This is the corresponding function for the system call for changing the number of tickets.
 * This is called from the library function set_priority().
 * It creates a process structure to determine the calling process for changing the number of tickets.
 * It then calls the system function defined schedule.c with the process structure and the number of tickets.
 */
int do_set_ticket(){
  ...
  /* accessing the number of tickets from the message structure. */
	int ticket = m_in.m1_i1;

  /* Storing the current process in the process structure. */
	rmp = mp;

  /* calling the scheduler function. */
	return sched_ticket(rmp,ticket);	
}
